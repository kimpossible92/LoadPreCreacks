...
public class CController : MonoBehaviour
{
    ...
    protected virtual void CastRaysAbove()
    {
        ...
        for (int i = 0; i < NumberOfVerticalRays; i++)
        {
            ...
            _aboveHitsStorage[i] = MMDebug.RayCast(rayOriginPoint, (transform.up), rayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, color1, Parameters.DrawRaycastsGizmos);
    ...
    protected virtual void CastRaysToTheSides(float raysDirection)
    {
        ...
        for (int i = 0; i < NumberOfHorizontalRays; i++)
        {
            ...
            if (State.WasGroundedLastFrame && i == 0)
            {
                ...
                _sideHitsStorage[i] = MMDebug.RayCast(rayOriginPoint, raysDirection * (transform.right), horizontalRayLength, PlatformMask, color1, Parameters.DrawRaycastsGizmos);
            }
            else
            {
                Color color2 = Color.white;
                _sideHitsStorage[i] = MMDebug.RayCast(rayOriginPoint, raysDirection * (transform.right), horizontalRayLength, PlatformMask & ~OneWayPlatformMask & ~MovingOneWayPlatformMask, color2, Parameters.DrawRaycastsGizmos);
            }
    ...
    protected virtual void StickToSlope()
    {
        ...
        _stickRaycast = MMDebug.RayCast(_stickRayCastOrigin, -transform.up, rayLength, PlatformMask, color1, Parameters.DrawRaycastsGizmos);
